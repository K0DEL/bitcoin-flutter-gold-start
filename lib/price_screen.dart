...
class _PriceScreenState extends State<PriceScreen> {
  ...
  void getData() async {
    try {
      isWaiting = true;
      dataMap = await CoinData().getCoinData(selectedCurrency);
      isWaiting = false;
      setState(() {
        for(int i=0;i<cryptoList.length;i++) {
          value[i] = dataMap[cryptoList[i]];
        }
      });
    } catch (e) {
      print(e);
  ...
  List<Widget> getCryptoCards(){
    ...
    for (int i=0;i<cryptoList.length;i++) {
      cryptoCards.add(CryptoCard(value: isWaiting == false ? '?' : value[i], selectedCurrency: selectedCurrency, cryptoCurrency: cryptoList[i],),);
    }
    return cryptoCards;
  }
